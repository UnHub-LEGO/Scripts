...
function RayfieldLibrary:GetGui()
	return rayfieldDestroyed
end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
		    ...
		    table.insert(connections, Button.Interact.MouseButton1Click:Connect(function()
		        ...
		        if rayfieldDestroyed then return end -- Stop if UI was destroyed
		    ...
		    function ButtonValue:GetButton()
		        return Button
		    end
		...
		function Tab:CreateTextEditor(TextEditorSettings)
		    local TextEditorValue = {}
		
		    local TextEditor = Instance.new("Frame")
		    TextEditor.Name = TextEditorSettings.Name
		    TextEditor.Parent = TabPage
		    TextEditor.Size = UDim2.new(1, -10, 0, 73)
		    TextEditor.BackgroundColor3 = Color3.new(0.137, 0.137, 0.137)
		
		    local UICorner = Instance.new("UICorner")
		    UICorner.CornerRadius = UDim.new(0, 9)
		    UICorner.Parent = TextEditor
		
		    local UIStroke = Instance.new("UIStroke")
		    UIStroke.Color = Color3.new(0.196, 0.196, 0.196)
		    UIStroke.Thickness = 1
		    UIStroke.Parent = TextEditor
		    
		    local ScrollingFrame = Instance.new("ScrollingFrame", TextEditor)
		    ...
		    TextBox.FocusLost:Connect(function()
				...
				if not Success then
					TweenService:Create(TextEditor, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
					TweenService:Create(UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
					print("Rayfield | "..TextEditorSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					TweenService:Create(TextEditor, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
				end
		    ...
		    function TextEditorValue:Destroy()
		        if TextEditor and TextEditor.Parent then
		            TextEditor:Destroy()
		            TextEditor = nil
		        end
		    end
		
		    function TextEditorValue:GetButton()
		        return TextEditor
		    end
		...
		function Tab:CreateParagraph(ParagraphSettings)
			...
			if ParagraphSettings.RichText then 
				Paragraph.Content.RichText = ParagraphSettings.RichText
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				local Success, Response = pcall(function()
					...
					ToggleSettings.Callback(ToggleSettings.CurrentValue, false)
				end)
			...
			function ToggleSettings:GetToggle()
				return Toggle
			end
			...
			function ToggleSettings:Set(NewToggleValue)
				...
				local Success, Response = pcall(function()
					...
					ToggleSettings.Callback(ToggleSettings.CurrentValue, true)
				end)
		...
		function Tab:CreateImageToggle(ToggleSettings)
			local ToggleValue = {}
			...
			Toggle.Title.Size = UDim2.new(0, 340, 0, 14)
			Toggle.Title.Position = UDim2.new(0, 60, 0.5, 0)
		    ...
		    ImageLabel.Image = tostring(ToggleSettings.Img)
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Interact.MouseButton1Down:Connect(function(X)
				...
				local Loop; Loop = RunService.Stepped:Connect(function()
					if SLDragging then
						...
						TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(0, Current - Slider.Main.AbsolutePosition.X, 1, 0)}):Play()
						...
						if SliderSettings.CurrentValue ~= NewValue then
							local Success, Response = pcall(function()
								SliderSettings.Callback(NewValue, false)
							end)
							if not Success then
			...
			function SliderSettings:Set(NewVal)
				...
				local Success, Response = pcall(function()
					SliderSettings.Callback(NewVal, true)
				end)
			...
			function SliderSettings:SetRange(NewMin, NewMax)
				NewMin = tonumber(NewMin)
				NewMax = tonumber(NewMax)
			
				if not NewMin or not NewMax or NewMin >= NewMax then
					warn("Invalid range values passed to SetRange")
					return
				end
				...
				local Relative = (NewValue - NewMin) / (NewMax - NewMin)
				local SizeX = Slider.Main.AbsoluteSize.X * Relative
				SizeX = math.max(SizeX, 5)
			
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {
	...
	function Window:CreateChatTab(Name, Image, Ext, TextHolder, img, ButtonSettings)
		...
		TabPage.Name = "Frame"
		TabPage.Parent = Rayfield.Main.Elements
		TabPage.Position = UDim2.new(0.5, 0, 0.5, 0)
		TabPage.Size = UDim2.new(1, -25, 1, 0)
		...
		TextButton.ZIndex = 1
		TextButton.Text = [[]]
		TextButton.TextColor3 = Color3.new(0.10588236153125763, 0.16470588743686676, 0.20784315466880798)
		...
		for _, TemplateElement in ipairs(TabPage.ScrollingFrame:GetChildren()) do
			if TemplateElement.ClassName == "Frame" and TemplateElement.Name ~= "Placeholder" then
				TemplateElement:Destroy()
			end
		end
		...
		table.insert(connections, Button.Interact.MouseButton1Click:Connect(function()
		    local Success, Response = pcall(ButtonSettings.Callback1)
		    if rayfieldDestroyed then return end
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Title.Text = ColorPickerSettings.Name
			ColorPicker.Visible = true
			ColorPicker.Parent = TabPage.ScrollingFrame
			ColorPicker.Size = UDim2.new(1, -10, 0, 45)
			Background.Size = UDim2.new(0, 39, 0, 22)
			Display.BackgroundTransparency = 0
			Main.MainPoint.ImageTransparency = 1
		...
		function Tab:CreateSection(SectionName)
			...
			if SDone then
				local SectionSpace = Elements.Template.SectionSpacing:Clone()
				SectionSpace.Visible = true
				SectionSpace.Parent = TabPage.ScrollingFrame
			end

			local Section = Elements.Template.SectionTitle:Clone()
			Section.Title.Text = SectionName
			Section.Visible = true
			Section.Parent = TabPage.ScrollingFrame
		...
		function Tab:CreateDivider()
			...
			local Divider = Elements.Template.Divider:Clone()
			Divider.Visible = true
			Divider.Parent = TabPage.ScrollingFrame
		...
		function Tab:CreateChatSend(ParagraphSettings)
			local ParagraphValue = {}
			local debounce = false

			local Paragraph1 = Elements.Template.Paragraph:Clone()
			Paragraph1.Parent = TabPage.ScrollingFrame
			Paragraph1.BackgroundTransparency = 1
			for i, v in pairs(Paragraph1:GetChildren()) do
				v:Destroy()
			end
			
			local Paragraph2 = Elements.Template.Paragraph:Clone()
			Paragraph2.Title:Destroy()
			Paragraph2.Content.Text = ParagraphSettings.Content
			Paragraph2.Content.Size = UDim2.new(1, -10, 0, 0)
			Paragraph2.Visible = true
			Paragraph2.Parent = Paragraph1
			Paragraph2.Position = UDim2.new(0.3, 0, 0, 0)
			Paragraph2.Size = UDim2.new(0.7, 0, 0, 0)

			Paragraph2.BackgroundTransparency = 1
			Paragraph2.UIStroke.Transparency = 1
			Paragraph2.Content.TextTransparency = 1
			...
			TweenService:Create(Paragraph2, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
			TweenService:Create(Paragraph2.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			TweenService:Create(Paragraph2.Content, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()	

			function ParagraphValue:Set(NewParagraphSettings)
				Paragraph2.Content.Text = NewParagraphSettings.Content
			end
		...
		function Tab:CreateChatReply(ParagraphSettings)
			local ParagraphValue = {}
			local debounce = false
			local Layout = 0
			...
			function ParagraphValue:CreateContent(ContentValue)
				local Content = Paragraph.Content:Clone()
				Content.Parent = Paragraph
				Content.Name = "Content_Layout_"..tostring(Layout)
				Content.Text = ContentValue.Content
				Content.LayoutOrder = Layout
				Content.TextTransparency = 0
				Layout = Layout + 1
			end
			
			function ParagraphValue:CreateCodeBox(CodeBoxValue)
				...
				Script.Position = UDim2.new(0, 10, 0.5, 0)
				Script.Size = UDim2.new(0, 50, 0, 0)
				Script.AnchorPoint = Vector2.new(0, 0.5)
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.Title.Text = InputSettings.Name
			Input.Visible = true
			Input.Parent = TabPage.ScrollingFrame
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			Dropdown.Title.Text = DropdownSettings.Name
			Dropdown.Visible = true
			Dropdown.Parent = TabPage.ScrollingFrame
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			Keybind.Title.Text = KeybindSettings.Name
			Keybind.Visible = true
			Keybind.Parent = TabPage.ScrollingFrame
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Title.Text = ToggleSettings.Name
			Toggle.Visible = true
			Toggle.Parent = TabPage.ScrollingFrame
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Title.Text = SliderSettings.Name
			Slider.Visible = true
			Slider.Parent = TabPage.ScrollingFrame
			...
			Slider.Main.Interact.MouseButton1Down:Connect(function(X)
				...
				local Loop; Loop = RunService.Stepped:Connect(function()
					if SLDragging then
						...
						TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(0, Location - Slider.Main.AbsolutePosition.X > 5 and Location - Slider.Main.AbsolutePosition.X or 5, 1, 0)}):Play()
						Loop:Disconnect()
					end
				end)
			end)
